...
AddEventHandler('fuel:refuelFromPump',function(pumpObject,ped,vehicle)
	...
	while isFueling do
		...
		if not pumpObject then
			DrawText3Ds(vehicleCoords.x,vehicleCoords.y,vehicleCoords.z + 0.5,"Натисни ~g~E ~w~за да откажеш")
			DrawText3Ds(vehicleCoords.x,vehicleCoords.y,vehicleCoords.z + 0.34,"Бензиностанция: ~b~"..Round(GetAmmoInPedWeapon(ped,883325847) / 4500 * 100,1).."%~w~    Гориво: ~y~"..Round(currentFuel,1).."%")
	...
	while true do
		...
		while not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) do
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped),-1) == ped then
				...
				DrawText3Ds(pumpCoords.x,pumpCoords.y,pumpCoords.z + 1.2,"Излез от ~y~колата~w~за да заредиш")
			else
				local vehicle = GetPlayersLastVehicle()
				local vehicleCoords = GetEntityCoords(vehicle)
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped),vehicleCoords) < 3.5 then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle,-1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 99 and canFuel then
							DrawText3Ds(stringCoords.x,stringCoords.y,stringCoords.z + 1.2,"Натисни ~g~E ~w~за да заредиш")
						...
						elseif not canFuel then
							...
							DrawText3Ds(stringCoords.x,stringCoords.y,stringCoords.z + 1.2,"~g~ПЪЛЕН РЕЗЕРВОАР")
						end
					end
