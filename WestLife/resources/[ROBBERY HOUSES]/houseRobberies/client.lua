...
local robbableItems = {
 [1] = {x = 1.90339700, y = -3.80026800, z = 1.29917900, name = "в хладилника", isSearched = false},
 [2] = {x = -3.50363200, y = -6.55289400, z = 1.30625800, name = "в чекмеджета", isSearched = false},
 [3] = {x = -3.50712600, y = -4.13621600, z = 1.29625800, name = "на масата", isSearched = false},
 [4] = {x = 8.47819500, y = -2.50979300, z = 1.19712300, name = "в склада", isSearched = false},
 [5] = {x = 9.75982700, y = -1.35874100, z = 1.29625800, name = "в склада", isSearched = false},
 [6] = {x = 8.46626300, y = 4.53223600, z = 1.19425800, name = "в гардероба", isSearched = false},
 [7] = {x = 5.84416200, y = 2.57377400, z = 1.22089100, name = "на масата", isSearched = false},
 [8] = {x = 3.04164100, y = 0.31671810, z = 3.58363900, name = "на секцията", isSearched = false},
 [9] = {x = 6.86376900, y = 1.20651200, z = 1.36589100, name = "под леглото", isSearched = false},
 [10] = {x = 2.03442400, y = -5.61585100, z = 3.30395600, name = "в шкафа", isSearched = false},
 [11] = {x = -5.53120400, y = 0.76299670, z = 1.77236000, name = "в кутията", isSearched = false},
 [12] = {x = -1.24716200, y = 1.07820500, z = 1.69089300, name = "на масата", isSearched = false},
 [13] = {x = 2.91325400, y = -4.2783510, z = 1.82746400, name = "на масата", isSearched = false},
}
...
Citizen.CreateThread(function ()
 while true do
	...
	for v in pairs (robbedHouses) do
		robbedHouses[v] = nil
	end
 end
end)
...
AddEventHandler('houseRobberies:attempt', function(lockpicks)
 if isRobbing and DoesEntityExist(safe) then
  ...
  if GetDistanceBetweenCoords(playerCoords, safepos.x, safepos.y, safepos.z, true) <= 3.0 then
   ...
   local policeChance = math.random(1,10)
   if policeChance > 6 then
 ...
 if lockpicks > 0 and isNight() and not isRobbing then
  local playerCoords = GetEntityCoords(PlayerPedId(), true)
  local count = getTableSize(robbedHouses)
  if count == 0 then
	for id,v in pairs(robbableHouses) do
      ...
      TriggerServerEvent('houseRobberies:removeLockpick')
			...
			exports["esx-taskbar"]:taskBar(12000,"Разбиване..")
			isLockpicking = false
			pedSpawned = false
		
			

			table.insert(robbedHouses,id)
		...
		for id,v in pairs(robbableHouses) do
				if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
					if not inTable(robbedHouses,id) then
            TriggerServerEvent('houseRobberies:removeLockpick')
							TriggerEvent('lockpickAnimation')
              AlertThePolice()
              AlertThePolice1()
...
function AlertThePolice()
    ...
    TriggerEvent('police:houseRobbery')
		--[[exports['core_dispach']:addCall("20-12", "Обир на къща", {
		{icon="fas fa-map-marked-alt", info=streetName},
		}, {PlayerCoords.x,PlayerCoords.y,PlayerCoords.z}, "police", 3000, 11, 5 )]]--

end
function AlertThePolice1()
	local PlayerData = ESX.GetPlayerData()
	local playerPed = PlayerPedId()
	PedPosition		= GetEntityCoords(playerPed)
    ...
    TriggerEvent('police:houseRobbery')
		--[[exports['core_dispach']:addCall("20-12", "Обир на къща", {
		{icon="fas fa-map-marked-alt", info=streetName},
		}, {PlayerCoords.x,PlayerCoords.y,PlayerCoords.z}, "sheriff", 3000, 11, 5 ]]--
...
-- end
 ...
 while isRobbing do
  ...
  if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
   showMessage('Натисни ~g~E~w~ за да излезнеш')
...
AddEventHandler('houseRobberies:deleteHouse', function(id)
 ...
 SetEntityAsNoLongerNeeded(robberyped1)
  ...
  if isRobbing then
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Търси ~g~[Z]~s~')
	...
	local playerPed = PlayerPedId()
	   ...
	   FreezeEntityPosition(playerPed,true)
		exports["esx-taskbar"]:taskBar(10000,"Търсене...")
   ...
   if disturbance > 95 then
    if not calledin then
     local num = 200 - disturbance
...
AddEventHandler('safe:success', function()
 ...
 safepos = {["x"] = 0, ["y"] = 0, ["z"] = 0}
 TriggerServerEvent('houseRobberies:giveMoneySafe')
 TriggerServerEvent('houseRobberies:searchItem')
