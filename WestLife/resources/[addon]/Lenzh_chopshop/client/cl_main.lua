...
function ChopVehicle()
    ...
    elseif GetGameTimer() - Timer > Config.CooldownMinutes * 60000 then
        ...
        if TotalTime >= 0 then
            ESX.ShowNotification("Върни се след " ..TotalTime.. " минути")
...
function VehiclePartsRemoval()
    ...
    if rearLeftDoor ~= -1 then
        if ChoppingInProgress == true then
            ESX.ShowNotification("Отваряне на Задна Лява Врата")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 2, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Сваляне на Задна Лява Врата")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false), 2, true)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Отваряне на Задна Дясна Врата")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 3, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Сваляне на Задна Дясна Врата")
            Citizen.Wait(Config.RemovePart)
    ...
    if bonnet ~= -1 then
        if ChoppingInProgress == true then
            ESX.ShowNotification("Отваряне на Капака")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 4, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Сваляне на Капака")
            Citizen.Wait(Config.RemovePart)
    ...
    if boot ~= -1 then
        if ChoppingInProgress == true then
            ESX.ShowNotification("Отваряне на Багажника")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 5, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Сваляне на Багажника")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false),5, true)
        end
    end
    Citizen.Wait(1000)
    ESX.ShowNotification("Пешо ще ти помогне с разглабянето!")
    Citizen.Wait(Config.RemovePart)
    if ChoppingInProgress == true then
        local vehicle =  GetVehiclePedIsIn( ped, false )
        if vehicle then
