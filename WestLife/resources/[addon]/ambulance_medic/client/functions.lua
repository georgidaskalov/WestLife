...
TryToGetMedicalTreatment = function(medicIndex)
    ...
    Heap.ESX.TriggerServerCallback("chames_ambulance_medic:requiredAmount", function(required)
        if required then
            ...
            Heap.ESX.ShowNotification("В момента има медици на смяна, говорете с тях първо.")
        end
    end)
end
...
GetMedicalTreatment = function(medicIndex)
    ...
    if not medic.Sequence then return Heap.ESX.ShowNotification("Няма път за медика, свържете се с Стаф или Дев.") end
    if not medic.Beds then return Heap.ESX.ShowNotification("Няма свободни легла, свържете се с Стаф или Дев.") end
...
BedThread = function(medicIndex, bed, bedHandle)
    ...
    while GetGameTimer() - Heap.InBed < Default.HelpTime do
        ...
        DrawScriptText(bed.Location, "В момента ви ~b~лекуват~s~ " .. timePercent .. "%")
    end
    ...
    Heap.ESX.ShowNotification("Излекуваха ви, върнете се пак , когато имате нужда от помощ.")
end
