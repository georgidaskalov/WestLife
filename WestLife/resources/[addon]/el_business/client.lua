...
function OpenCreateBusinessMenu()
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_create',
    {
        ...
        elements = {
            ...
            {label = 'Име: ', action = 'name', value = '', key = 2},
            {label = 'Описание: ', action = 'desc', value = '', key = 3},
            {label = 'Адрес: ', action = 'address', value = '', key = 4},
            {label = 'Име на блип: ', action = 'blipname', value = '', key = 5},
            {label = 'Цена: ', action = 'price', value = '', key = 6},
            {label = 'Печалба: ', action = 'earnings', value = '', key = 7},
            {label = 'Цена на стока: ', action = 'stock_price', value = '', key = 8},
            {label = 'Такса: ', action = 'taxrate', value = '', key = 9},
            {label = '<span style="color:red;">Таксата е в % 0.1=10% - 1.0=100%</span>', action = ''},
            {label = 'Създай бизнес', action = 'create'},
            {label = 'Изтрий бизнес', action = 'discard'}
        }
    }, function(data,menu)
...
AddEventHandler("el_business:businessList",function()
    if counttbl(businessData)<1 then print("No business info on client, if there is some on server, you should try running the `/business reload` command to sync server info") else
        ...
        for k,v in pairs(businessData) do
            print("-"..tostring(v["id"]))
            print("|-> Име: "..v["name"])
            print("|-> Адрес: "..v["address"])
            print("|-> Описание: "..v["description"])
            print("|-> Име на блип: "..(v["blipname"]~=nil and v["blipname"] or Config.blip.name))
            print("|-> Собственик: "..(v["owner_rp_name"]~=nil and v["owner_rp_name"] or "N/A").." ("..(v["owner"]~=nil and v["owner"] or "N/A")..")")
            print("|-> Цена: "..v["price"].."$")
            print("|-> Цена на стока: "..v["stock_price"].."$")
            print("L-> Печалба: "..v["earnings"].."$/h")
        end
