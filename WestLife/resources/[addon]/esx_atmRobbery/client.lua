...
AddEventHandler('esx_atmRobbery:outlawNotify', function(alert)
	if isPlayerWhitelisted then
		TriggerEvent('chat:addMessage', { args = { "^5 Обир: " .. alert }})
	end
end)
			...
			if not HackingATM then
				for k,v in pairs(Config.ATMs) do
					if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, true) <= 1.5 then
						DrawText3Ds(v.x,v.y,v.z, "Натисни ~g~[G]~s~ за да ~r~обереш~s~  ~y~банкомата~s~")
...
function hackingEvent(success)
    ...
    if success then
        ...
        ESX.ShowNotification("Вие ~g~успешно~s~ хакнахте ~y~банкомата~s~")
    else
		ESX.ShowNotification("Вие се ~r~провалихте~s~ да хакнете ~y~банкомата~s~!")
	end
...
function SendDistressSignal()
	...
	ESX.TriggerServerCallback('gksphone:namenumber', function(Races)
		...
		TriggerServerEvent('gksphone:jbmessage', 'Обир на банкомат', Races[1].phone_number, 'Обир на банкомат', '', GPS, 'police')
	end)
end
