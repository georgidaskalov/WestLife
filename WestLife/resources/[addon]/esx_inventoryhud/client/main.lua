...
AddEventHandler('esx:onPlayerDeath', function(data)
	...
	--fastWeapons[1] = nil
	--fastWeapons[2] = nil
	--fastWeapons[3] = nil   uncomment those if broken
	--fastWeapons[4] = nil
	--fastWeapons[5] = nil
end)
...
RegisterNUICallback("PutIntoFast", function(data, cb)
	if string.find(data.item.name, "ammo_") then
		exports['mythic_notify']:DoLongHudText('error', 'Не може амуниции')
		return
	end
	if not data.item.usable and not string.find(data.item.name, "WEAPON_") then
		exports['mythic_notify']:DoLongHudText('error', 'Може само предмети/оръжия които можеш да използваш')
		return
	end
        ...
        for i = 1, #players, 1 do
            if players[i] ~= PlayerId() then
                ...
                table.insert(
                    elements,
                    {
                        label = "WestLifeRP", --GetPlayerName(players[i]),
        ...
        for i = 1, #players, 1 do
            if players[i] ~= PlayerId() then
                if GetPlayerServerId(players[i]) == data.player then
                    playAnim('mp_common', 'givetake1_a', 2000)
                    exports['pogressBar']:drawBar(2000, "Даване....")
                    foundPlayer = true
                end
            end
        end
