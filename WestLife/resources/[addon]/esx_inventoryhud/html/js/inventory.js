...
window.addEventListener("message", function (event) {
    if (event.data.action == "display") {
        ...
        else if (type === "shop") { $(".info-div").show();}
		
        else if (type === "jobstash") { $(".info-div").show();}
		
        else if (type === "gang") { $(".info-div").show();}
...
function ItemUsed(alerts) {
    ...
    $('#use-alert').hide('slide', {direction: 'up'}, 500, function () {
...
function makeDraggables(){
	$('#itemFast-1').droppable({
        drop: function (event, ui) {
            ...
            if (type === "normal" || type === "glovebox" || type === "drops" && (itemInventory === "main" || itemInventory === "fast" || itemInventory === "second")) {
	...
	$('#itemFast-2').droppable({
        drop: function (event, ui) {
            ...
            if (type === "normal" || type === "glovebox" || type === "drops" && (itemInventory === "main" || itemInventory === "fast")) {
                disableInventory(500);
                $.post("http://esx_inventoryhud/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot : 2
...
function shopInventorySetup(items) {
    ...
    $.each(items, function (index, item) {
        ...
        $("#otherInventory").append('<div class="slotfix"><div id="itemOther-' + index + '" class="item" style = "background-image: url(\'img/items/' + item.name + '.png\')">' +
            '<div class="item-count">' + count + '</div> <div class="item-name">' + item.label + ' - <span style="color: green; font-size: 12px;">' + cost + '</span></div> </div ><div class="item-name-bg"></div></div>');
        $('#itemOther-' + index).data('item', item);
        $('#itemOther-' + index).data('inventory', "second");
    });
}
...
$(document).ready(function () {
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            } else if (type === "jobstash" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://esx_inventoryhud/TakeFromJobStash", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
    ...
    } else if (type === "jobstash" && itemInventory === "main") {
		disableInventory(500);
		$.post("http://esx_inventoryhud/PutIntoJobStash", JSON.stringify({
			item: itemData,
			number: parseInt($("#count").val())
		}));
