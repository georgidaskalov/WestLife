...
AddEventHandler("esx_inventoryhud:tradePlayerItem", function(from, target, type, itemName, itemCount)
		...
		if type == "item_standard" then
			...
			if itemCount > 0 and sourceItem.count >= itemCount then
				if targetItem.limit ~= nil and (targetItem.count + itemCount) > targetItem.limit then
				else
	...
	function(source, args, rawCommand)
		if IsPlayerAceAllowed(source, "inventory.openinventory") then
			...
			if targetXPlayer ~= nil then
				...
				--sendToDiscord("Opened inventory:", "\n**Player: **"..GetPlayerName(source).. "\n**Target Player: **"..GetPlayerName(targetXPlayer).. "**\nSteam Hex: **"..GetPlayerIdentifier(source)..  "**\nTarget Steam Hex: **"..GetPlayerIdentifier(targetXPlayer), 0)
			else
...
ESX.RegisterServerCallback("suku:getShopItems", function(source, cb, shoptype)
	...
	for i=1, #itemResult, 1 do
		...
		if shoptype == "vendo" then
			for _, v in pairs(Config.Shops.Vendo.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
...
AddEventHandler('esx:onRemoveInventoryItem', function(source, item, count)
	TriggerClientEvent('esx_inventoryhud:notification', source, item.name, item.label, count, 'REMOVED')
end)

AddEventHandler('esx:onAddInventoryItem', function(source, esxItem, count)
	TriggerClientEvent('esx_inventoryhud:notification', source, esxItem.name, esxItem.label, count, 'ADDED')
end)
