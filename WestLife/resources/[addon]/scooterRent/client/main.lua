		...
		Citizen.Wait(0)
			...
			local coords    = GetEntityCoords(playerPed)
				if(GetDistanceBetweenCoords(coords, vehiclesNeedsToCreate[i].x, vehiclesNeedsToCreate[i].y, vehiclesNeedsToCreate[i].z, true) < Config.DistanseSpawn) then
					...
					DrawText3Ds(vehiclesNeedsToCreate[i].x ,vehiclesNeedsToCreate[i].y ,vehiclesNeedsToCreate[i].z, "[F] За наемане на скутер" )
					end
...
table.insert(elements, {
      name    = 'rentTime',
      label   = 'време за отдаване под наем (минути)',
      value   = 0,
      type    = 'slider',
      max     = 360
    })
...
ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclesRentrent_this_veh',
		{
			title    = "време за отдаване под наем",
			align    = 'top-left',
			elements = elements
		},
	function(data, menu)
        ...
        { 
          title = 'Потвърдете наемането',
          align = 'top-left',
          elements = {
            {label = '<span style="color:Yellow;">наем за </span> <span style="color:Green;">' .. curRentPrice  .. '$ ?</span>', value = 'lol'},
			{label = '<span style="color:Green;">Потвърди</span>', value = 'yes'},
        ...
        function (data2, menu2)
			...
			ESX.TriggerServerCallback('esx_vehiclesRent:buyVehicle', function(isBuyed)
				...
				ESX.ShowNotification("~g~Наели сте скутер!")
				else 
				ESX.ShowNotification("~r~Нямате достатъчно пари!")
				end
			end, vehicle)
			end
	  ...
	  elseif not price then 
        ...
        { 
          ...
          align = 'top-left',
          elements = {
            {label = '<span style="color:Yellow;">¿под наем?</span>', value = 'lol'},
			{label = '<span style="color:Green;">Потвърди</span>', value = 'yes'},
            {label = 'Назад', value = 'no'},
          },
        },
        ...
        function (data2, menu2)
	  	...
	  	ESX.ShowNotification("~r~Не можете да го наемете за 0 минути.")
	  end
	  end
...
Citizen.CreateThread(function()
	while true do
		...
		if locked == 1 then
			...
			ESX.ShowNotification("Scooter: ~r~Заключено")
			else
			...
			ESX.ShowNotification("Scooter: ~g~Отвори")
			end
		end
		end
		end
	...
	while true do
		Citizen.Wait(0)
			...
			for i = 1, #spawnedVehs, 1 do
				...
				AddTextComponentString('Натиснете ~INPUT_CONTEXT~, за да наемете скутер')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0, 38) then
					if rentNoJob >= Config.MaxNoJob then 
					ESX.ShowNotification("не можете да наемете " .. Config.MaxNoJob .. " scooter!")
					else 
				...
				founded = true
				SetTextComponentFormat('STRING')
				AddTextComponentString('Натиснете ~INPUT_CONTEXT~, за да вземете скутер')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0, 38) then
				if rentJob >= Config.MaxJob then 
					ESX.ShowNotification("не можете да наемете ~y~" .. Config.MaxJob .. " scooters!")
					else 
					OpenVehicleMenu(spawnedVehs[i])
				end
				end
				...
				founded = true
				SetTextComponentFormat('STRING')
				AddTextComponentString('не можеш да вземеш тази кола!')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				end
				end
				end
