...
RegisterCommand("changejob", function (src, args, raw)
    if timer == 0 and allowCommand then
        ...
        ESX.ShowNotification('Трябва да изчакате 30 секунди преди да смените работата си,сега трябва да изчакате: (време в секунди) '..timer) -- Here you can change whatewer you want
    end
end, false)
...
AddEventHandler('sqz_switchjob:returnSecondJob', function(secondjob, secondjob_grade)
    ...
    ESX.ShowNotification('Смени си работата.') -- Here you can change whatewer you want
    Wait(5000)
    ESX.ShowNotification('Сегашната ти работа е: '..ESX.PlayerData.job.label..' И ранка ти е: '..ESX.PlayerData.job.grade_label) -- Here you can change whatewer you want
end)
