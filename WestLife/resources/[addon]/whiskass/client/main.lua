...
Citizen.CreateThread(function()
	...
	TriggerServerEvent('whiskass:checkIfSentenced')
end)
...
RegisterNetEvent('whiskass:inCommunityService')
AddEventHandler('whiskass:inCommunityService', function(actions_remaining)
	...
	while actionsRemaining > 0 and communityServiceFinished ~= true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z) > 45 then
			ESX.Game.Teleport(playerPed, Config.ServiceLocation)
				...
				TriggerServerEvent('whiskass:produljiDaGalishKote')
				actionsRemaining = actionsRemaining + Config.ServiceExtensionOnEscape
		end

	end

	TriggerServerEvent('whiskass:prikluchiDaGalishKote', -1)
...
RegisterNetEvent('whiskass:prikluchiDaGalishKote')
AddEventHandler('whiskass:prikluchiDaGalishKote', function(source)
			...
			for i = 1, #availableActions do
				...
				if distance < 1.5 then
					...
					if(IsControlJustReleased(1, 38))then
						...
						TriggerServerEvent('whiskass:zavurshiDaGalishKote')
