...
function callTakeHostage()
	...
	if not IsEntityDead(ped) then
		...
		if not takeHostage.InProgress and canTakeHostage then			
			...
			if closestPlayer then
				...
				if targetSrc ~= -1 then
					...
					drawNativeNotification("~r~Няма никои ,който да вземете!")
				end
			else
		...
		drawNativeNotification("~r~Не можеш да го хванеш, докато си мъртъв!")
	end
end 
...
Citizen.CreateThread(function()
	while true do 
		...
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Натиснете [G] ,за да го пуснете, [H] ,за да го убиете")
