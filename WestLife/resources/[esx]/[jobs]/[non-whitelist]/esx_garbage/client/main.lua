					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							if iscurrentboss then
								...
								trashcollection = false
								truckdeposit = false
								ESX.ShowNotification("Завършената работа се върнете в камиона!")
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
			if dist <= 2.0 then
			ESX.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "[~g~E~s~] Хвърли чантата в камиона.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag <= 0 then
					if iscurrentboss then
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] почистете отломките", 1.0)
					else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Събирането приключи.. Изчакайте в камиона..", 1.0)
					end
				else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Съберете торба за боклук ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
...
Citizen.CreateThread(function()
	while true do
		...
		while debugit do
			Citizen.Wait(0)
			plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
			ESX.Game.Utils.DrawText3D(plyCoords + vector3(0.0, 0.0, 0.1), "Идент. № на работния камион: "..tostring(work_truck), 0.8)
			ESX.Game.Utils.DrawText3D(plyCoords + vector3(0.0, 0.0, 0.0), "Действие за проверка на натискане на бутон: "..tostring(CollectionAction), 0.8)
			ESX.Game.Utils.DrawText3D(plyCoords + vector3(0.0, 0.0, -0.1), "Депо за камиони: "..tostring(truckdeposit), 0.8)
			ESX.Game.Utils.DrawText3D(plyCoords + vector3(0.0, 0.0, -0.2), "Събиране на кошчета: "..tostring(trashcollection), 0.8)
		end
	end
end)
