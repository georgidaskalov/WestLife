...
Citizen.CreateThread(function()
	while true do
		...
		if Spppprzedaje then
			...
			ESX.Game.Utils.DrawText3D(Pozycja, "Остават ~b~"..Sssekundy.."~s~ докато разпродажбата приключи ...", 0.9)
			if Sssekundy < 1 then
				Spppprzedaje = false
				Sssekundy = 60
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if Kopie then
			...
			ESX.Game.Utils.DrawText3D(Pozycja, "Събира ~b~Картофени~s~ култури", 0.6)
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
        ...
        if GetDistanceBetweenCoords(coords, 1946.670, 5018.533, 42.952, true) < 15.0 and ploned1 == true then
          ...
          ESX.Game.Utils.DrawText3D(Pozycja, "Натисни [~g~E~s~] за събиране на реколта", 0.5)
        end
        if GetDistanceBetweenCoords(coords, 1941.715, 5022.883, 42.634, true) < 15.0 and ploned2 == true then
          ...
          ESX.Game.Utils.DrawText3D(Pozycja2, "Натисни [~g~E~s~] за събиране на реколта", 0.5)
        end
        if GetDistanceBetweenCoords(coords, 1940.599, 5013.583, 42.634, true) < 15.0 and ploned3 == true then
          ...
          ESX.Game.Utils.DrawText3D(Pozycja3, "Натисни [~g~E~s~] за събиране на реколта", 0.5)
        end
        if GetDistanceBetweenCoords(coords, 1938.436, 5020.075, 43.060, true) < 15.0 and ploned4 == true then
          ...
          ESX.Game.Utils.DrawText3D(Pozycja4, "Натисни [~g~E~s~] за събиране на реколта", 0.5)
        end
        if GetDistanceBetweenCoords(coords, 1930.202, 5004.469, 43.728, true) < 20.0 and ploned5 == true then
          ...
          ESX.Game.Utils.DrawText3D(Pozycja5, "Натисни [~g~E~s~] за събиране на реколта", 0.5)
        end
				...
				if GetDistanceBetweenCoords(coords, 1940.599, 5013.583, 42.634, true) < 2.0 and ploned3 == true then
					...
					if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'farmer' then
						...
						RemoveBlip(pole3)
						local rand = math.random(5, 10)
						TriggerServerEvent('CenturionFarmerJob:giveItem', 'Картофи', rand)
						PlaySound(-1, "PICK_UP", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
						PlaySoundFrontend(-1, "MEDAL_GOLD", "HUD_AWARDS", 0);
						--TriggerEvent('genesis-energy:takeEnergy', math.random(5000, 15000))
						ploned3 = false
					...
					if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'farmer' then
						if not IsPedInAnyVehicle(GetPlayerPed(-1), false) and not maAuto then
							...
							if HaveDMV then
								...
								Notif('НЕ може да шофирате!Нямате шофьорска книжка!')
							end
						elseif not IsPedInAnyVehicle(GetPlayerPed(-1), false) and maAuto then
							Notif('Вече имате изкарано превозно средство!')
						end
					end

					if IsControlJustReleased(0, 38) and IsPedSittingInVehicle(GetPlayerPed(-1), jobVehicle) then
						Notif('Превозното средство е паркирано! Сега можете да изберете друг!')
						Notif('Благодарим ви за работата!')
						maAuto = false
				...
				if GetDistanceBetweenCoords(coords, 428.073, 6477.219, 27.860, true) < 2.0 and IsPedSittingInVehicle(GetPlayerPed(-1), jobVehicle) then
					...
					if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'farmer' then
						...
						TriggerEvent("pNotify:SendNotification", {text = 'Разтоварването е в процес ...'})
						Spppprzedaje = true
						DrawSubtitleTimed(30000, 1)
						FreezeEntityPosition(jobVehicle, true)
						SetVehicleUndriveable(jobVehicle, true)
						Notif('Реколтата бавно се възстановява! Върнете се във фермата, за да ги вземете!')
						maAuto = false
						Notif('Ще можете да замените превозното средство!')
						Citizen.Wait(30000)
						Spppprzedaje = false
						Sssekundy = 60
			...
			if PlayerDoingJob and GetDistanceBetweenCoords(coords, 2010.932, 4969.550, 41.572, true) < 40.0 then
				if not IsPedInAnyVehicle(GetPlayerPed(-1), false) and not maAuto then
					...
					ESX.Game.Utils.DrawText3D(Pozycja, "Натиснете [~g~E~s~], за да вземете автомобила за доставка", 0.8)
        ...
        ESX.Game.Utils.DrawText3D(Pozycja, "Натисни [~g~E~s~] ,за да разтоварите товара", 1.0)
			end
...
function reGrow()
	Citizen.Wait(300000)
	Notif('Реколтата е готова за бране!')
	SetupBlips()
end
...
function JobMapBlip()
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("3. Превозно средство")
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 2012.865, 4994.723, 41.173, true) < 10.0 and PlayerData.job ~= nil and PlayerData.job.name == 'farmer' then
			ESX.ShowHelpNotification('Натисни ~INPUT_CONTEXT~, за да започнете фермерската работа!')
			local Pozycja = {
				["x"] = 2012.865,
				["y"] = 4994.723,
				["z"] = 41.173
			}
			ESX.Game.Utils.DrawText3D(Pozycja, "Натисни [~g~E~s~] за да започнете изкопаването", 0.6)
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'farmer' and GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 2012.865, 4994.723, 41.173, true) < 2.0 then
				if PlayerDoingJob then
					if PlayerDoingJob then
						Notif('Вече сте започнали работа!')
					end
				else
					...
					if canWork then
						--SetupJob()
						PlayerDoingJob = true
						Notif('Ние подготвяме вашето работно място!')
						PlaySoundFrontend(-1, "10s", "MP_MISSION_COUNTDOWN_SOUNDSET", 0)
						Wait(10000)
						--PlaySoundFrontend(-1, "DRUG_TRAFFIC_AIR_BOMB_DROP_ERROR_MASTER", 0, 1)
						Notif('Започвате работа! Отидете до близкото поле, за да приберете реколтата')
						SetupJob()
					elseif not canWork then
						Notif('Изглеждаш уморен! Трябва да си починете!')
					end
				end
			end
		end
