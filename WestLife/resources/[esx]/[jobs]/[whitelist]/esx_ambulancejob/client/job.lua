...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		...
		elements = {
			...
			{label = 'Издаване на фактура', value = 'billing'}
		}
	}, function(data, menu)
		if data.current.value == 'billing' then
			local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
			if closestPlayer == -1 or closestDistance > 3.0 then
			ESX.ShowNotification('Няма хора наоколо')
			else
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				title = 'Стойност на фактурата'
			}, function(data, menu)

				local amount = tonumber(data.value)
				if amount == nil then
					ESX.ShowNotification('Невалидна стойност')
				else
					menu.close()
					local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Болница', amount)
						ESX.ShowNotification('Фактурата е успешно написана')
					end
		...
		elseif data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer == -1 or closestDistance > 2.0 then  -- was 1.0 
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		align    = 'bottom-right',
		elements = {
			{label = _U('pharmacy_take', _U('medikit')), item = 'medikit', type = 'slider', value = 1, min = 1, max = 15},
			{label = _U('pharmacy_take', _U('bandage')), item = 'bandage', type = 'slider', value = 1, min = 1, max = 15}
		}
	}, function(data, menu)
