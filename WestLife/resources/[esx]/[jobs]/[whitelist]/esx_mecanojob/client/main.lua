...
local allowedTowModels = { 
  ['16ramrbc'] = { x = 0.4, y = -1.3, z = -0.7 }, -- default GTA V flatbed
}
          ...
          function(data, menu)
            ...
            if amount == nil then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mecano', _U('mecano'), amount)
              end
            end
          end,
        function(data, menu)
    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            local elements = {
              {label = 'Репатрак', value = '16ramrbc'},
              {label = 'Джип', value = 'orangef150'},
			        {label = 'Бусче', value = 'nspeedo'},
              {label = 'Кука', value = 'f450s'},
              --{label = ('Pickup'), value = 'f450towtruk'},
             --s {label = ('Small Trailer'), value = 'orangef150'},
			  --{label = 'Company Caddy', value = 'f350worktruck'},
