...
RegisterNetEvent("ws:fine")
AddEventHandler("ws:fine", function()
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
		...
		if amount == nil then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				TriggerEvent("gas_station:Notify","importante",'Няма никой наблизо')
			else
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanic', 'mechanic', amount)
				TriggerEvent("gas_station:Notify","importante",'Фактурата е написана')
			end
