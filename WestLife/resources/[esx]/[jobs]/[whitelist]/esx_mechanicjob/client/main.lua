		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					--{label = 'Голем камион', value = 'hdwrecker'},
					--{label = 'Влекач с кука 1', value = 'f450towtruk'},
					{label = 'Влекач с кука 2', value = 'f450s'},
					{label = 'Джип', value = 'orangef150'}
				}
...
function OpenMobilemechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		align    = 'top-left',
		elements = {
			{label = '🔧 Поправи кола',        value = 'fix_vehicle1'},
			--{label = '🚗 Wheel Repair',        value = 'fix_wheel'},
			{label = '🧽 Почисти',         value = 'clean_vehicle'},
			{label = '🔐 Конфискувай',       value = 'del_vehicle'},
			{label = '⚙️ Издърпай',      value = 'dep_vehicle'},
			{label = '📝 Фактура',       value = 'billing'},
		--	{label = '👨‍🏭 Повикай колега',       value = 'meca1'},	
			{label = '📣 Отворено ',     value = 'mechanic_ouvert'},
			{label = '📣 Затворено',     value = 'mechanic_ferme'},
			--{label = '📣 Ad Pause',     value = 'mechanic_pause'},
			--{label = '📣 Ad Promo',     value = 'mechanic_promo'},
			{label = '🔨 Отключи',        value = 'hijack_vehicle'},
			{label = '🛠️ Поправка на мотор',        value = 'fix_vehicle'},
			
			{label = '📦 Предмети', value = 'object_spawner'},
			--{label = '🔋 Nitro', value = 'nitro'},
			--{label = '🏷 Номер', value = 'platemenu'}
	}}, function(data, menu)
		...
		if data.current.value == 'fix_wheel' then
			TriggerEvent('rgz_tirereplacement:fix')	
			menu.close()
	end			
		...
		elseif data.current.value == 'mechanic_ouvert' then
			TriggerServerEvent('AnnouncemechanicOuvert')
		elseif data.current.value == 'mechanic_ferme' then
			TriggerServerEvent('AnnouncemechanicFerme')
		elseif data.current.value == 'mechanic_pause' then
			TriggerServerEvent('AnnouncemechanicPause')
		elseif data.current.value == 'mechanic_promo' then
			TriggerServerEvent('AnnouncemechanicPromo')
	...
	elseif data.current.value == 'fix_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				exports['progressBars']:startUI(20000, "Поправка")
				Citizen.Wait(20000)
		...
		elseif data.current.value == 'clean_vehicle' then
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
				exports['progressBars']:startUI(10000, "Почистване..")
				Citizen.Wait(10000)
					...
					if targetVehicle ~= 0 then
						if not IsPedInAnyVehicle(playerPed, true) then
							if vehicle ~= targetVehicle then
								AttachEntityToEntity(targetVehicle, vehicle, 20, -0.0, -6.0, 0.8, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
		...
		elseif data.current.value == 'object_spawner' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions_spawn', {
				...
				align    = 'top-left',
				elements = {	
				{label = 'Бариера', value = 'prop_barrier_work06a'},				
				...
				{label = 'Стол', value = 'v_ret_gc_chair02'},			
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			if NPCOnJob then
				if GetGameTimer() - NPCLastCancel > 10 * 60000 then
					StopNPCJob(true)
