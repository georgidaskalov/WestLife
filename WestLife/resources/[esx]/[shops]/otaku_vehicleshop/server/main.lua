...
MySQL.ready(
	function()
		Categories = MySQL.Sync.fetchAll("SELECT * FROM vehicle_categories_npc ORDER BY label ASC")
		local vehicles = MySQL.Sync.fetchAll("SELECT * FROM vehicles_npc")
		...
		for i = 1, #vehicles, 1 do
			...
			if (vehicle.hash == "" or vehicle.hash == "0") then
				...
				MySQL.Async.execute(
					"UPDATE vehicles_npc SET hash = @hash WHERE model = @model",
					{
						["@hash"] = vehicle.hash,
						["@model"] = vehicle.model
					}
