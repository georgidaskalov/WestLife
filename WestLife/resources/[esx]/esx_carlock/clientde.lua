...
Citizen.CreateThread(function()
  ...
  while true do
    Citizen.Wait(0)
	if (IsControlJustPressed(1, 58)) then --182 = Key L
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("Около теб няма автомобил , който може да бъде заключен.")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification('Автомобилът ~y~'..vehicleLabel..'~s~ ~r~е заключен~s~.')
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification('Автомобилът е ~y~'..vehicleLabel..'~s~ ~g~отключен~s~.')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
