    ...
    while true do
        ...
        if NearBank() and not isBankOpened and NearBank() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("Натисни ~INPUT_PICKUP~ за да отвориш ~b~банката")
            else
            	inRange = true
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['okokTextUI']:Open('[E] За да влезеш в акаунта', 'darkblue', 'left') 
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if NearATM() and not isBankOpened and NearATM() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("[E] За да влезеш в акаунта")
            else
            	inRange = true
            end
