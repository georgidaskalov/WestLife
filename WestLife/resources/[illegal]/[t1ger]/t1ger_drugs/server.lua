...
ESX.RegisterUsableItem('drugItem', function(source)
	...
	if not HasCooldown(GetPlayerIdentifier(source)) then
		if xPlayer.getInventoryItem(Config.HackerDevice).count >= 1 then
			...
			TriggerClientEvent('esx:showNotification', source, "Трябва ти ~r~Хакерски Лаптоп~s~ за да включиш ~y~флашката~s~")
		end
 	else
	 	TriggerClientEvent("esx:showNotification",source,string.format("~y~Флашката~s~ може да се използва след: ~b~%s минути~s~",GetCooldownTime(GetPlayerIdentifier(source))))
  	end
end)
...
AddEventHandler("t1ger_drugs:GetSelectedJob", function(drugType,BuyPrice,minReward,maxReward)
	...
	if xPlayer.getMoney() >= BuyPrice then
		...
		TriggerClientEvent("esx:showNotification",source,"Ти плати ~g~$"..BuyPrice.."~s~ за ~r~"..label.."~s~ поръчката")
	else
		TriggerClientEvent("esx:showNotification",source,"Нямаш достатъчно пари!")
	end
end)
	...
	for k,v in pairs(Config.DrugEffects) do 
		ESX.RegisterUsableItem(v.UsableItem, function(source)
			...
			if not DrugEffect(GetPlayerIdentifier(source)) then
				...
				TriggerClientEvent("esx:showNotification",source,string.format("Ти вече ~b~употребяваш~s~ наркотик",GetDrugEffectTime(GetPlayerIdentifier(source))))	
			end	
		end)
	end
end)
	...
	for k,v in pairs(Config.DrugConversion) do 
		ESX.RegisterUsableItem(v.UsableItem, function(source)
			...
			if not reqItems then
				...
				TriggerClientEvent("esx:showNotification",source,"~r~Нямаш~s~ достатъчно ~y~"..reqItemLabel.."~s~")
				return
			end
			...
			if xPlayer.getInventoryItem(v.RewardItem).count <= v.MaxRewardItemInv.f or (not scale and xPlayer.getInventoryItem(v.RewardItem).count <= v.MaxRewardItemInv.e) then
				if not Converting(GetPlayerIdentifier(source)) then
					...
					TriggerClientEvent("esx:showNotification",source,string.format("Ти все още ~b~пакетираш~s~",GetConvertTime(GetPlayerIdentifier(source))))	
				end	
			else
				TriggerClientEvent("esx:showNotification",source,"~r~Нямаш~s~ достатъчно ~b~място~s~ за ~y~"..itemLabel.."~s~")
			end
		end)
	end
end)
...
AddEventHandler('t1ger_drugs:DrugJobInProgress', function(targetCoords, streetName)
	TriggerClientEvent('t1ger_drugs:outlawNotify', -1,string.format("^0Стрелба на ^5%s^0",streetName))
...
AddEventHandler('t1ger_drugs:DrugSaleInProgress', function(targetCoords, streetName)
	TriggerClientEvent('t1ger_drugs:outlawNotify', -1,string.format("^0Продажба на наркотици ^5%s^0",streetName))
	TriggerClientEvent('t1ger_drugs:OutlawBlipEvent', -1, targetCoords)
end)
...
AddEventHandler("t1ger_drugs:sellDrugs", function()
	...
	elseif heroin > 0 then
		...
		TriggerClientEvent('esx:showNotification', source, "~r~Нямаш~r~ достатъчно ~y~наркотици~s~ в теб")
		return
	end
	...
	if drugType ~= nil then
		...
		TriggerClientEvent('esx:showNotification', source, "Ти продаде ~b~"..drugamount.."x~s~ ~y~"..drugLabel.."~s~ за ~r~$"..price.."~s~")
	end		
end)
...
AddEventHandler("t1ger_drugs:canSellDrugs", function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer ~= nil then
		local soldAmount = (xPlayer.getInventoryItem("coke1g").count > 0 or xPlayer.getInventoryItem("meth1g").count > 0 or xPlayer.getInventoryItem("joint2g").count > 0 or xPlayer.getInventoryItem("heroin").count > 0) and CheckSoldAmount(xPlayer.getIdentifier()) < Config.maxCap
