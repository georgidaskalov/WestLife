            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Вземете парите", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Вземете парите", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Вземете парите", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" or PlayerData.job.name == "sheriff" then
        exports["mythic_notify"]:SendAlert("inform", "Алармата е задействана!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
            ...
            if PlayerData.job.name == "police" and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Отключване на вратата", 0.80)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Заключване на вратата", 0.80)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    exports["mythic_notify"]:SendAlert("error", "ВРАТИТЕ ЩЕ СЕ ЗАТВОРЯТ СЛЕД 30 СЕКУНДИ!")
    Citizen.Wait(100000)
    exports["mythic_notify"]:SendAlert("error", "ВРАТИТЕ СЕ ЗАТВАРЯТ!")
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports['progressBars']:startUI(4500, "Вкарване на картата...")
    Citizen.Wait(1000)
    ...
    disableinput = false
    Citizen.Wait(1000)
    Process(UTK.hacktime, "Хакване...")
    exports["mythic_notify"]:SendAlert("success", "Хакването бе успешно!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    exports["mythic_notify"]:SendAlert("error", "Имате 2 минути преди да се активира охранителната аларма")
...
function SecondDoor(data, key)
    ...
    TaskStartScenarioInPlace(ped, "PROP_HUMAN_ATM", 0, true)
    exports['progressBars']:startUI(5500, "Използване на зловредна карта...")
    Citizen.Wait(1500)
    ...
    ClearPedTasksImmediately(ped)
    disableinput = false
    Process(3500, "Проникване в панела...")
    exports["mythic_notify"]:SendAlert("success", "Достъпът е осъществен!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "[~r~E~w~] Използвайте картата за достъп", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports["mythic_notify"]:SendAlert("error", "Нямате карта за достъп")
                        end
                    end)
                end
            end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~r~E~w~] Започнете обира", 0.90)
