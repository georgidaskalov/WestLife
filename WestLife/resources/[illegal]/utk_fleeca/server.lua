...
local DISCORD_NAME = "Малка Банка-logs"
...
AddEventHandler("utk_fh:startcheck", function(bank)
    ...
    if copcount >= UTK.mincops then
        if item >= 1 then
		    ...
		    sendToDiscord("Малка Банка ", "\n Беше обрана от \n**Player: **"..GetPlayerName(source).. "**\nSteam Hex: **"..GetPlayerIdentifier(source), 65280)
                    TriggerClientEvent("utk_fh:outcome", _source, true, bank)
                    TriggerClientEvent("utk_fh:policenotify", -1, bank)
                else
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Тази банка вече е била обрана. Трябва да изчакате! "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Тази банка се обира в момента")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "Нямате карта за достъп")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "Няма достатъчно полицаи в града")
    end
end)
