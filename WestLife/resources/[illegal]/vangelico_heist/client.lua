...
function SendDistressSignal()
	...
	ESX.TriggerServerCallback('gksphone:namenumber', function(Races)
		...
		TriggerServerEvent('gksphone:jbmessage', 'Обир на бижутерия', Races[1].phone_number, 'Обир на бижутерия', '', GPS, 'police')
	end)
end
...
AddEventHandler('av_vangelico:notify', function()
	if PlayerData.job.name == Config.PoliceJobName or PlayerData.job.name == Config.SheriffJobName then
		ESX.ShowNotification(Config.Lang['police'])
		ESX.ShowNotification(Config.Lang['sheriff'])
		local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1), false))
		SendDistressSignal()
		--TriggerServerEvent('esx_phone:send', 'police', ' Обир на бижутерия ', true, {x =x, y =y, z =z})
		--TriggerServerEvent('esx_phone:send', 'sheriff', ' Обир на бижутерия ', true, {x =x, y =y, z =z})
...
positions = {
    ...
    {{-620.8885, -224.6078, 37.0569, 0}, {-618.2340, -242.4096, 51.7851, 0},{36,237,157}, "Покрив"}, -- Outside the Sheriff's Station
...
Citizen.CreateThread(function ()
    while true do
        ...
        for _,location in ipairs(positions) do
            ...
            DrawMarker(1, loc1.x, loc1.y, loc1.z, 0, 0, 0, 0, 0, 0, 0, 0, 0, Red, Green, Blue, 200, 0, 0, 0, 0)
            DrawMarker(1, loc2.x, loc2.y, loc2.z, 0, 0, 0, 0, 0, 0, 0, 0, 0, Red, Green, Blue, 200, 0, 0, 0, 0)
