...
local DISCORD_NAME = "bijuteriq-logs"
...
AddEventHandler('av_vangelico:gas', function()
	...
	sendToDiscord("Бижутерия", "\n Беше обрана от **Player: **"..GetPlayerName(source).. "**\nSteam Hex: **"..GetPlayerIdentifier(source), 65280)
...
ESX.RegisterServerCallback('av_vangelico:cooldown', function(source,cb)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == Config.PoliceJobName or xPlayer.job.name == Config.SheriffJobName then
			cops = cops + 1
		end
	end
