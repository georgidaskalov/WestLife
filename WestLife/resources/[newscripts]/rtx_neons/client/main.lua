...
Citizen.CreateThread(function ()
    while ESX == nil do
		TriggerEvent(Config.EsxTriggerName, function(obj) ESX = obj end)
        Citizen.Wait(0)
    end
end)

if Config.NeonsUseOnlyViaController then
	RegisterNetEvent("rtx_neons:UseController")
	AddEventHandler("rtx_neons:UseController", function()
		if player == nil then
			player = PlayerPedId()
		end
		if vehicle == nil then
		...
		if IsPedSittingInAnyVehicle(player) and GetPedInVehicleSeat(vehicle, -1) == player then
			ESX.TriggerServerCallback("rtx_neons:CheckNeons", function(neons)
				if tonumber(neons) == 1 then
					SendNUIMessage({
						message    = "showui"
					})
					...
					Notify("This vehicle doesnt have special neons!")
				end
			end, vehicleprops)	
		end		
	end)
