...
if GetConvar('voice_useNativeAudio', 'false') == 'true' then
	...
	Cfg.voiceModes = {
		{1.5, "Шептене"}, -- Whisper speech distance in gta distance units
		{3.0, "Нормално"}, -- Normal speech distance in gta distance units
		{6.0, "Викане"} -- Shout speech distance in gta distance units
	}
else
	Cfg.voiceModes = {
		{3.0, "Шептене"}, -- Whisper speech distance in gta distance units
		{7.0, "Нормално"}, -- Normal speech distance in gta distance units
		{15.0, "Викане"} -- Shout speech distance in gta distance units
	}
end
