...
function joinRadioChannel(walkie, channelData)
	...
	if walkie or not Config.useItem then
		...
		elseif channelData < 1 then
			showNotification("Не може да се свържете с тази честота")
			return
		end
	...
	if haveItem then
		if tonumber(playerInRadioChannel) == channel and not Config.voipScript == "pma" and not Config.voipScript == "toko" then
			showNotification("Вече сте на тази честота")
		else
			setPlayerRadioChannel(channel, walkie)
		end
	else
		showNotification("Нямате радиостанция.")
	end
end
	...
	if Config.framework then
		...
		if job then
			...
			elseif Config.voipScript == "pma" then
				...
				exports["pma-voice"]:setRadioVolume(70.0)
			...
			showNotification("Свързахте се с честота "..channel..".00 MHz")
		else
			showNotification("Не можете да се свържете с тази честота")
		end
	else
		local job = true
		for i=1, #Config.restrictChannel do
...
function radioLeave(walkie)
	...
	if haveItem then
		...
		if not Config.voipScript == "pma" and not Config.voipScript == "toko" and (playerInRadioChannel == nil or tonumber(playerInRadioChannel) == 0) then
			...
			showNotification("Излезнахте от честота "..playerInRadioChannel.. ".00 MHz")
		end
	else
		showNotification("Нямате радиостанция.")
	end
		...
		if Config.useItem then
			TriggerCallback('tgiann:item-control', function(qtty)
				if qtty > 0 then
					...
					showNotification("Нямате радио станция") 
				end
			end, Config.ItemName)
		else
			enableRadio(true)
