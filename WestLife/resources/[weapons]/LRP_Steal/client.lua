...
Citizen.CreateThread(function()
 ...
 local ped = PlayerPedId()
		...
		if IsControlJustPressed(1, Keys["H"]) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and not IsPedInAnyVehicle(ped, true) then
			if CheckIsPedDead() then  
				exports['mythic_notify']:DoCustomHudText('error', 'Жертвата е мъртва.',4000)
 ...
 local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
         ...
         if closestPlayer ~= -1 and closestDistance <= 1.5 then
			 ...
			 if distance <= 1.5 then
                 ...
                 if IsEntityPlayingAnim(searchPlayerPed, 'missminuteman_1ig_2', 'handsup_enter', 3) or IsEntityDead(searchPlayerPed) or GetEntityHealth(searchPlayerPed) <= 0 then

		             TriggerServerEvent('robo:jugador', target_id, playerheading, playerCoords, playerlocation)
					  Citizen.Wait(4500)
				else
				exports['mythic_notify']:DoCustomHudText('error', 'Не е вдигнал ръце.')	
...
AddEventHandler('robo:doarrested', function()
	...
	exports['progressBars']:startUI(3500, " Конфискуване ")	
	Citizen.Wait(3000)
